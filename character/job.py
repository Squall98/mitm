...
def recole(character, cell):
    ...
    if character.map.carreau[cell]["cell"].entity == [] or character.map.carreau[cell]["cell"].entity.type == "Player":
        ...
        while True:
            # Tente de se déplacer vers la cellule
            if character.deplacement.deplacement(character.cell.CellID,
                                                 from_pos_x_y_to_cell_id(close_cell[1][1], close_cell[1][0],
                                                                         character.map.mapswidth),
                                                 character.map.mapswidth, character.map.carreau,
                                                 character.map.binairemap, character.map.sun, character.map.resource)[
                0]:
                break
            else:
