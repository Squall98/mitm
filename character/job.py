...
def recole(character, cell):
    ...
    if character.map.carreau[cell]["cell"].entity == [] or character.map.carreau[cell]["cell"].entity.type == "Player":
        ...
        while True:
            success = character.deplacement.deplacement(
                character.cell.CellID,
                from_pos_x_y_to_cell_id(close_cell[1][1], close_cell[1][0], character.map.mapswidth),
                character.map.mapswidth,
                character.map.carreau,
                character.map.binairemap,
                character.map.sun,
            ...
            if success:
                ...
                if close_cell == (1000, None):
                    character.map.carreau[cell]["cell"].isInteractive = False  # A changer
                    return False
                bad_cell.append(close_cell[0])  # Ajoutez l'ID de la cellule, pas le tuple complet
                close_cell = get_close_cell(character.map, cell, bad_cell)
