    ...
    def fight(self):
        ...
        if self.map_frame.character.PM:
            ...
            if lauch_spell_before:
                ...
                self.map_frame.character.deplacement.deplacement(self.map_frame.character.cell.CellID, good_pos,
                                                                 self.map_frame.character.map.mapswidth,
                                                                 self.map_frame.character.map.carreau,
                                                                 self.map_frame.character.map.binairemap,
                                                                 self.map_frame.character.map.sun,
                                                                 self.map_frame.character.map.resource, pm, True)
                self.launch_spell()
            else:
        ...
        if False:  # dist > 8 and dist not in (1.9142135623730951, 1.5):
            dist = 99999
            good_pos = from_cell_id_to_x_y_pos(self.map_frame.character.cell.CellID,
                                               self.map_frame.character.map.mapswidth)
            for noob in self.map_frame.entity_gestion.entity:
                if noob.id == int(self.map_frame.character.id_):
                    continue
                pos_entity = from_cell_id_to_x_y_pos(int(noob.cell), self.map_frame.character.map.mapswidth)
                x = heuristic(pos_entity, good_pos)
                if x < dist:
                    dist = x
                    good_pos = pos_entity
            ...
            good_pos = from_pos_x_y_to_cell_id(good_pos[0], good_pos[1], self.map_frame.character.map.mapswidth)
            pm = int(self.map_frame.character.PM)
            self.map_frame.character.deplacement.deplacement(self.map_frame.character.cell.CellID,
                                                             from_pos_x_y_to_cell_id(good_pos[0], good_pos[1],
                                                            self.map_frame.character.map.mapswidth),
                                                             self.map_frame.character.map.mapswidth,
                                                             self.map_frame.character.map.carreau,
                                                             self.map_frame.character.map.binairemap,
